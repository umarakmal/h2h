...
const HelpForm = () => {
    ...
    const handleInputChange = (index, field, value) => {
        ...
        var sameDateValidation = updatedConcerns.some(abc => abc.concernof === value)
        let toasterDisplayed = false;
        if (sameDateValidation === true) {
            if (!toasterDisplayed) {
                toast.error("Concern of date cannot be the same")
                toasterDisplayed = true
            }
            return
        }

        //To check weather user has already raised request same date and same issue
        const checkApi = async () => {
            ...
            if (formData.belongsTo === '') {
                ...
                try {
                    const response = await fetch(`${url}/api/check-same-concerndate`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json'
                        },
                        body: JSON.stringify({ belongsTo, issue, concernof, requestby })
                    });
                    ...
                    if (response.status === 200) {
                        if (data === true) {
                            toast.error("Already raised the request")
                            return
                        } else {
    ...
    const handleSubmit = async (event) => {
        ...
        if (formData.belongsTo === '') {
            ...
            if (requester_mobile_no.length < 10) {
                toast.error("Mobile number should be of 10 digits")
                return false
            } else {
                ...
                const requestby = isAuth().result1.EmployeeID
                const AH = isAuth().result1.account_head
                const DOD = isAuth().result1.DOD
                const DOJ = isAuth().result1.DOJ
                const alt_mobile = isAuth().result1.altmobile
                ...
                const response = await fetch(`${url}/api/raise-issue`, {
                    ...
                    body: JSON.stringify({ requester_mobile_no, issue, mobile_no, concern, belongsTo, requestby, communicated_with, AH, Process, name, reportto, location, designation, cm_id, DOD, DOJ, alt_mobile })
                });
    ...
    const issueChange = (e) => {
        ...
        setFormData({
            ...
            communicated_with: formData.communicated_with,
            requester_mobile_no: formData.requester_mobile_no,
